...
var util_1 = require("../analyzer/util");
/**
...
function getUsedRefs(schema) {
    return (0, util_1.query)(["$..['$ref']"], schema);
}
/**
...
function accumulateUsedRefs(schema, $refs, $ref) {
    ...
    getUsedRefs($refSchema).forEach(function (_a) {
        var currRef = _a.value;
...
function reducer(definition, opts) {
    ...
    var reduceTags = 'tags' in opts ? opts.tags.map(function (tag) { return tag.toLowerCase(); }) : [];
    var reducePaths = 'paths' in opts
        ? Object.entries(opts.paths).reduce(function (acc, _a) {
            var key = _a[0], value = _a[1];
            var newKey = key.toLowerCase();
            var newValue = Array.isArray(value) ? value.map(function (v) { return v.toLowerCase(); }) : value.toLowerCase();
            acc[newKey] = newValue;
            return acc;
        }, {})
        : {};
    ...
    if ((0, utils_1.getAPIDefinitionType)(definition) !== 'openapi') {
        ...
        Object.keys(reduced.paths).forEach(function (path) {
            ...
            Object.keys(reduced.paths[path]).forEach(function (method) {
                ...
                if (reduceTags.length) {
                    ...
                    else if (!operation.tags.filter(function (tag) { return reduceTags.includes(tag.toLowerCase()); }).length) {
                        delete reduced.paths[path][method];
                        return;
                    }
                }
                ...
                getUsedRefs(operation).forEach(function (_a) {
                    var ref = _a.value;
                    $refs.add(ref);
                });
